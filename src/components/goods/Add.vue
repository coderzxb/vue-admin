				    ...
				    <el-tab-pane label="商品参数" name="1">
						...
						:key="item.attr_id">
							...
							<el-checkbox-group v-model="item.attr_vals">
							    <el-checkbox :label="cb" border v-for="(cb,i) in item.attr_vals" :key="i"/>
							</el-checkbox-group>
						</el-form-item>
				    </el-tab-pane>
  ...
  data () {
    return {
    	...
    	addFormRules:{
            ...
            goods_cat:[
    			{ required: true, message: '请选中三级分类', trigger: 'blur' }
            ]
    	},
    	// 分类列表数据
    	cateList:[],
    	...
    	onlyTableData:[],
    	// 图片上传的URL地址
    	uploadURL:'http://nodeshop.shelterc.top:3300/api/private/v1/upload',
    	// 图片上传的headers请求头
    	headerObj:{
  	...
  	async tabClicked(){
  		...
  		if (this.activeIndex === '1') {
  			...
  			if (res.meta.status!==200) {
	  			...
	  			// 获取的attr_vals数据转为数组;再循环遍历
  				res.data.forEach(item => {
  					item.attr_vals = item.attr_vals.length === 0 ? []:item.attr_vals.split(' ');
  				});
	  			this.manyTableData = res.data;
	  		}
  		}else if (this.activeIndex === '2') { // 证明访问的商品属性
  			const {data:res} = await this.$http.get(`categories/${this.addForm.goods_cat[2]}/attributes`,
  			{params: { sel:'only' } });
  			if (res.meta.status!==200) {
